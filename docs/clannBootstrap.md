...
The output directory is the Eolas data root. Its default is `./data`, so a
Clann named `Example Clann` is written beneath
`data/clanns/example-clann/`. Creation is atomic and refuses to overwrite a
non-empty Clann directory.
...
createdPath = clannCreate(clann, Path("/private/eolas-data"))
```
